        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.content, { paddingBottom: 86 + insets.bottom + 24 }]}>
          ...
          <View style={styles.summaryCard}>
            <View style={styles.ticketRow}>
              ...
              <Text style={styles.ticketTotal} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.76}>{money(estimatedTotal)}</Text>
            </View>
            <View style={styles.divider} />
            <View style={styles.totalRow}><Text style={styles.totalLabel}>Estimated total</Text><Text style={styles.totalValue} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.76}>{money(estimatedTotal)}</Text></View>
      ...
      <View style={[styles.payBarOuter, { bottom: Math.max(insets.bottom, 14) }]}>
        <View style={styles.payBar}>
          ...
          <Pressable disabled={!canContinue} style={[styles.payButton, !canContinue && styles.payButtonDisabled]} onPress={() => void handlePay()}>
            {startingPayment ? <ActivityIndicator color="#ffffff" /> : <><Text style={styles.payButtonText} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72}>{paymentMethod === "bank_transfer" ? "Get bank details" : paymentMethod === "card" ? "Continue securely" : "Send payment prompt"}</Text><Lock size={18} color="#ffffff" /></>}
          </Pressable>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  payBarOuter: { position: "absolute", left: 14, right: 14 }, payBar: { minHeight: 86, borderRadius: 26, borderWidth: 1, borderColor: BORDER, backgroundColor: "#fff", padding: 13, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12, shadowColor: "#13285f", shadowOpacity: 0.18, shadowRadius: 26, shadowOffset: { width: 0, height: 12 }, elevation: 14 }, payTotalWrap: { flex: 1, minWidth: 0 }, payLabel: { color: MUTED, fontSize: 10, fontWeight: "900", letterSpacing: 1.2 }, payAmount: { color: TEXT, fontSize: 22, fontWeight: "900", marginTop: 4, flexShrink: 1 }, payButton: { flex: 1, minWidth: 0, maxWidth: "64%", minHeight: 58, borderRadius: 18, backgroundColor: ACCENT, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingHorizontal: 12 }, payButtonDisabled: { backgroundColor: "#cfd4df" }, payButtonText: { color: "#fff", fontSize: 13, fontWeight: "900", textAlign: "center", flexShrink: 1 },
  loadingOverlay: { ...StyleSheet.absoluteFill, zIndex: 50, elevation: 50, backgroundColor: "rgba(14,24,52,0.42)", alignItems: "center", justifyContent: "center", paddingHorizontal: 26 },
