...
function getUploadFileMeta(asset: UploadAsset) {
  ...
  return {
    name: `seller-storefront-${Date.now()}.${ext === "heic" || ext === "heif" ? "jpg" : ext}`,
    type: mime,
  };
}

export async function uploadStorefrontImage(asset: UploadAsset) {
