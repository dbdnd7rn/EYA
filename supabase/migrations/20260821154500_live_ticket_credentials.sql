...
-- Live ticket entry credentials are intentionally separate from payment
-- fulfilment. Fulfilment owns ticket issuance; this table owns short-lived
-- admission credentials for already-issued tickets.
...
-- No direct client reads/writes. Authenticated clients interact only through
...
create or replace function public.issue_ticket_live_credential(
  p_ticket_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_ticket.user_id <> v_user_id then
    raise exception 'This ticket does not belong to the signed-in user.';
  end if;

  if v_ticket.status <> 'active' or v_ticket.checked_in_at is not null then
    raise exception 'This ticket is not available for entry.';
  end if;

  v_expires_at := v_now + interval '60 seconds';
  v_token := 'EYA-LIVE-1-' || upper(encode(extensions.gen_random_bytes(24), 'hex'));
  ...
  if v_has_existing then
    ...
    set
      ...
      previous_token_hash = case
        when v_existing.current_token_hash is not null
         and v_existing.current_expires_at is not null
         and v_existing.current_expires_at > v_now
          then v_existing.current_token_hash
        else null
      end,
      previous_manual_hash = case
        when v_existing.current_manual_hash is not null
         and v_existing.current_expires_at is not null
         and v_existing.current_expires_at > v_now
          then v_existing.current_manual_hash
        else null
      end,
      previous_expires_at = case
        when v_existing.current_expires_at is not null
         and v_existing.current_expires_at > v_now
          then least(v_existing.current_expires_at, v_now + interval '15 seconds')
        else null
      end,
...
begin
  ...
  if v_method not in ('qr', 'manual') then
    raise exception 'Unsupported check-in method.';
  end if;

  if v_raw = '' then
    raise exception 'Live credential is required.';
  end if;

  if v_method = 'manual' then
    v_hash := encode(extensions.digest(upper(v_raw), 'sha256'), 'hex');
  else
  ...
  where (
    v_method = 'qr'
    and (
      (tlc.current_token_hash = v_hash and tlc.current_expires_at > v_now)
      or
    ...
    v_method = 'manual'
    and (
      (tlc.current_manual_hash = v_hash and tlc.current_expires_at > v_now)
      or
  ...
  if not found then
    raise exception 'Live credential is invalid or expired.';
  end if;

  if p_event_id is not null and v_ticket.event_id <> p_event_id then
    raise exception 'Ticket is for another event.';
  end if;

  if v_ticket.status <> 'active' then
    raise exception 'Ticket is %.', v_ticket.status;
  end if;
  ...
  insert into public.ticket_checkins (
    ...
    jsonb_build_object(
      'credential_kind', 'eya_live_ticket',
  ...
  -- Kill both the current and overlap credentials immediately after admission.
  update public.ticket_live_credentials
  set
  ...
  from (
    select id, title, date_label, venue, city
    from public.ticket_events
    where id = v_ticket.event_id
  ) e;

  select to_jsonb(t) into v_tier
  from (
    select id, name, price_mwk
  ...
  from (
    select id, total_mwk, quantity, payment_status, paid_at
