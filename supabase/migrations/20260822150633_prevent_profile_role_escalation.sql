...
grant update (
  full_name, phone, onboarded, avatar_url, avatar_preset,
  first_name, last_name, surname, campus, area
...
begin
  ...
  values (
    ...
    'student', coalesce(new.raw_user_meta_data->>'phone', '')
  )
...
begin
  insert into public.profiles (
    ...
    nullif(trim(meta->>'phone'), ''), 'student', true
  )
...
begin
  insert into public.profiles (
    ...
    nullif(trim(coalesce(meta->>'surname', meta->>'last_name', '')), ''),
    nullif(trim(coalesce(meta->>'phone', '')), ''), 'student', true
  )
  on conflict (id) do update set
    email = excluded.email,
    full_name = coalesce(excluded.full_name, public.profiles.full_name),
