...
as $$
begin
  if not exists (select 1 from public.ticket_organization_finance_entitlements f where f.organization_id=new.organization_id) then
...
$$;
drop trigger if exists seed_first_ticket_organization_finance_owner_trigger on public.ticket_organizer_access_grants;
...
create or replace function public.current_ticket_finance_entitlement(
  p_user_id uuid,p_organization_id uuid,p_allow_suspended boolean default false
)
...
as $$
  ...
  where f.user_id=p_user_id and f.organization_id=p_organization_id and o.status='active'
...
create or replace function public.admin_set_ticket_organization_finance_entitlement(
  p_organization_id uuid,p_user_id uuid,p_role text,p_status text,p_note text default null
)
