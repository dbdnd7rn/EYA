...
begin
  ...
  if nullif(trim(p_image_url), '') is null or nullif(trim(p_hero_image_url), '') is null then
    raise exception 'Card and hero images are required.';
  end if;
...
begin
  ...
  if nullif(trim(p_title), '') is null or nullif(trim(p_date_label), '') is null
     or nullif(trim(p_venue), '') is null or nullif(trim(p_city), '') is null then
    raise exception 'Title, date, venue, and city are required.';
  end if;
  if nullif(trim(p_image_url), '') is null or nullif(trim(p_hero_image_url), '') is null then
    raise exception 'Card and hero images are required.';
  end if;
  ...
  update public.ticket_events set
    title = trim(p_title),
    category = coalesce(nullif(trim(p_category), ''), 'Music'),
    description = nullif(trim(p_description), ''),
    date_label = trim(p_date_label),
    starts_at = p_starts_at,
    ends_at = p_ends_at,
    venue = trim(p_venue),
    city = trim(p_city),
    image_url = trim(p_image_url),
    hero_image_url = trim(p_hero_image_url),
    status = 'draft',
    review_note = null,
    updated_at = now()
  where id = p_event_id;
...
begin
  ...
  if p_sale_ends_at is not null and p_sale_starts_at is not null and p_sale_ends_at <= p_sale_starts_at then
    raise exception 'Ticket sale end must be after sale start.';
  end if;
  ...
  if p_tier_id is null then
    ...
    if p_capacity_total < (v_tier.capacity_sold + v_tier.capacity_reserved) then
    ...
    update public.ticket_tiers set
      ...
      capacity_total = p_capacity_total, available = coalesce(p_available,true),
      sale_starts_at = p_sale_starts_at, sale_ends_at = p_sale_ends_at,
...
begin
  ...
  select count(*) into v_tier_count from public.ticket_tiers
  where event_id = p_event_id and available = true and capacity_total > 0;
...
begin
  ...
  values (
    ...
    v_event.status, v_status, nullif(trim(p_note),'')
  );
...
as $$
  ...
  from (
    select jsonb_build_object(
      ...
      'gross_sales_mwk', coalesce(stats.gross_sales_mwk,0),
    ...
    from public.ticket_events e
    left join lateral (
      select sum(t.capacity_total)::bigint as capacity_total,
             sum(t.capacity_sold)::bigint as capacity_sold,
             sum(t.capacity_reserved)::bigint as capacity_reserved
      from public.ticket_tiers t where t.event_id=e.id
    ) tiers on true
    left join lateral (
      select sum(o.quantity)::bigint as tickets_sold,
             sum(o.total_mwk)::numeric as gross_sales_mwk
      from public.ticket_orders o
      where o.event_id=e.id and o.payment_status='paid'
    ) stats on true
