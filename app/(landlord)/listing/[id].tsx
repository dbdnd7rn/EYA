...
export default function EditListingScreen() {
  ...
  useEffect(() => {
    const loadListing = async () => {
      ...
      const { data, error } = await supabase
        .from("listings")
        .select("id,title,listing_type,campus,area,city,price_from,total_rooms,room_types,description,contact_phone,contact_method,water_billing,electricity_billing,gender_policy,occupancy_mode,students_per_room,rules,amenities,image_urls,latitude,longitude,is_active")
        .eq("id", id)
      ...
      const row = data as ListingDb;
      setTitle(row.title ?? "");
      setListingType(row.listing_type ?? "hostel");
      setCampus(row.campus ?? "");
      setArea(row.area ?? "");
      setCity(row.city ?? "");
      setPriceFrom(row.price_from != null ? String(row.price_from) : "");
      setTotalRooms(row.total_rooms != null ? String(row.total_rooms) : "");
      setDescription(row.description ?? "");
      setContactPhone(row.contact_phone ?? "");
      setContactMethod(row.contact_method ?? "whatsapp");
      setWaterBilling(row.water_billing ?? "exclusive");
      setElectricityBilling(row.electricity_billing ?? "exclusive");
      setGenderPolicy(row.gender_policy ?? "both");
      setOccupancyMode(row.occupancy_mode ?? "single");
      setStudentsPerRoom(String(row.students_per_room ?? 2));
      setAmenities((row.amenities ?? []) as AmenityKey[]);
      setListingActive(Boolean(row.is_active));

      const existingRoomTypes = (row.room_types ?? []).filter(Boolean);
      setRoomTypes(existingRoomTypes.filter((x) => (ROOM_TYPE_OPTIONS as readonly string[]).includes(x)));
      setCustomRoomTypes(existingRoomTypes.filter((x) => !(ROOM_TYPE_OPTIONS as readonly string[]).includes(x)).join(", "));

      const existingRules = (row.rules ?? []).filter(Boolean);
      setRules(existingRules.filter((x) => (RULE_OPTIONS as readonly string[]).includes(x)));
      setCustomRules(existingRules.filter((x) => !(RULE_OPTIONS as readonly string[]).includes(x)).join(", "));

      setPhotos(((row.image_urls ?? []) as string[]).map((url, i) => ({
  ...
  const toggleActive = async () => {
    ...
    setInfoMsg(next ? "Listing activated." : "Listing deactivated.");
  };
    ...
    <SafeAreaView style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Pressable style={[styles.softBtn, (loading || deleting) && { opacity: 0.7 }]} onPress={toggleActive} disabled={loading || deleting}>
          <Text style={styles.softBtnText}>{listingActive ? "Deactivate listing" : "Activate listing"}</Text>
        </Pressable>
        <Pressable style={[styles.deleteBtn, (loading || deleting) && { opacity: 0.7 }]} onPress={deleteListing} disabled={loading || deleting}>
          <Text style={styles.deleteBtnText}>{deleting ? "Deleting..." : "Delete listing"}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  deleteBtn: { backgroundColor: "#fff0f6", borderRadius: 14, paddingVertical: 13, alignItems: "center", borderWidth: 1, borderColor: "#ffd4e3" },
  deleteBtnText: { color: "#b0003a", fontWeight: "900" },
});
