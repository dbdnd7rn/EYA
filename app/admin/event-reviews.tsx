...
export default function AdminEventReviewsScreen() {
  ...
  const act = async (event: AdminTicketReviewEvent, action: "approve" | "request_changes" | "reject") => {
    ...
    Alert.alert(label, message, [
      ...
      {
        text: approve ? "Approve + Publish" : label,
          ...
          try {
            ...
            if (approve) {
              Alert.alert(
                ...
                `Published successfully${result.approved_version_number ? ` as EYA approval version ${result.approved_version_number}` : ""}. Any material event or ticket change must return through EYA review.`,
              );
            }
          } catch (e: any) {
    ...
    <SafeAreaView style={styles.root}>
      ...
      >
        ...
        <View style={styles.securityNote}>
          ...
          <Text style={styles.securityText}>Admin approves the event and the exact ticket configuration together. Organizer submissions are never public or purchasable before this approval.</Text>
        </View>
        ...
        <Pressable style={styles.organizerAccessBtn} onPress={() => router.push("/admin/organizer-access" as any)}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.organizerAccessSub}>Invite, renew, expire, revoke or re-enable Organizer Workspace access.</Text>
          </View>
        </Pressable>
          ...
          return (
            <View key={event.id} style={styles.card}>
              ...
              <View style={styles.organizerBox}>
                ...
                <Text style={styles.organizerName}>{event.organizer?.full_name || event.organizer?.email || "Temporary organizer"}</Text>
              ...
              <View style={styles.tierList}>
                <View style={styles.ticketSectionHead}>
                  <View>
                    <Text style={styles.boxLabel}>TICKETS TO APPROVE</Text>
                    <Text style={styles.ticketSummary}>{event.tiers.length} type{event.tiers.length === 1 ? "" : "s"} · {totalCapacity.toLocaleString()} total capacity</Text>
                  </View>
                </View>
                  ...
                  <View key={tier.id} style={styles.tierRow}>
                    <View style={{ flex: 1, gap: 3 }}>
                      <View style={styles.tierTitleRow}>
                        ...
                        <View style={[styles.availabilityBadge, !tier.available && styles.unavailableBadge]}>
                          <Text style={[styles.availabilityText, !tier.available && styles.unavailableText]}>{tier.available ? "Available" : "Disabled"}</Text>
                        </View>
                      </View>
              ...
              <View style={styles.approvalLockBox}>
                <ShieldCheck size={18} color="#087443" />
                <Text style={styles.approvalLockText}>Approval creates an immutable EYA version of these event and ticket terms. Price, capacity, venue, date, ticket availability or sale-window changes cannot silently reuse this approval.</Text>
              </View>
              ...
              <View style={styles.actions}>
                ...
                <Pressable style={[styles.actionBtn, styles.approveBtn]} disabled={busyId === event.id} onPress={() => void act(event, "approve")}>
                  {busyId === event.id ? <ActivityIndicator size="small" color="#fff" /> : <CheckCircle2 size={16} color="#fff" />}<Text style={styles.approveText}>Approve Event + Tickets</Text>
                </Pressable>
              </View>
            </View>
          );
...
const styles = StyleSheet.create({
  ...
  organizerAccessTitle: { color: TEXT, fontSize: 14, fontWeight: "900" }, organizerAccessSub: { color: MUTED, fontSize: 11, lineHeight: 16, fontWeight: "700", marginTop: 2 },
  ...
  organizerBox: { backgroundColor: "#f7f8fc", borderRadius: 16, padding: 12, gap: 3 }, boxLabel: { color: MUTED, fontSize: 9, fontWeight: "900", letterSpacing: 0.8 }, organizerName: { color: TEXT, fontSize: 14, fontWeight: "900", marginTop: 2 }, smallText: { color: MUTED, fontSize: 11, lineHeight: 16, fontWeight: "700" },
  tierList: { gap: 8 }, ticketSectionHead: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, ticketSummary: { color: TEXT, fontSize: 12, fontWeight: "900", marginTop: 3 },
  tierRow: { borderRadius: 14, borderWidth: 1, borderColor: BORDER, padding: 11, flexDirection: "row", alignItems: "flex-start", gap: 10 }, tierTitleRow: { flexDirection: "row", alignItems: "center", gap: 7, flexWrap: "wrap" }, tierName: { color: TEXT, fontSize: 13, fontWeight: "900" }, tierPrice: { color: TEXT, fontSize: 13, fontWeight: "900", paddingTop: 2 },
  availabilityBadge: { borderRadius: 999, backgroundColor: "#e4f7ec", paddingHorizontal: 7, paddingVertical: 3 }, availabilityText: { color: "#087443", fontSize: 8, fontWeight: "900" }, unavailableBadge: { backgroundColor: "#fff0f0" }, unavailableText: { color: "#a32929" }, saleText: { color: "#53617d", fontSize: 10, lineHeight: 15, fontWeight: "800" },
  approvalLockBox: { borderRadius: 16, backgroundColor: "#e8f7ee", padding: 12, flexDirection: "row", alignItems: "flex-start", gap: 9 }, approvalLockText: { flex: 1, color: "#276346", fontSize: 11, lineHeight: 17, fontWeight: "800" },
